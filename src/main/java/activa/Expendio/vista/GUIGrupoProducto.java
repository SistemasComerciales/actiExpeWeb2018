...
public class GUIGrupoProducto extends GUIInterfazCatalogos {
    ...
    private void cargarDatosGeneral() {
        ...
        ArrayList<GrupoProducto> grupos = persistencia.consultarTodos();

        for (GrupoProducto grupo : grupos) {
            String[] datosFila = new String[dtmTablaGeneral.getColumnCount()];

            datosFila[columnaId] = String.valueOf(grupo.getId());

            datosFila[columnaCodigo] = grupo.getCodigo();
            datosFila[columnaNombre] = grupo.getNombre();
    ...
    protected void accionBotonAgregar() {// Insertando nueva grupo
        try {
            ...
            boolean valido = puedeInsertarOModificar(false, grupo);
            if (valido) {
                deshacerFiltroTablaGeneral();

                boolean accion = grupo.insertar(usuario);
                cargarDatosGeneral();

                String mensaje = null, titulo = null, tipo = null;
                if (accion) {
                    mensaje = "Se ha insertado satisfactoriamente el grupo.";
                    titulo = "Insertado";
                    tipo = GUIJOption.mensajeInformacion;
                } else {
                    mensaje = "Ha ocurrido un error y no se ha podido insertar el grupo. Por favor inténtelo de nuevo.";
                    titulo = "Error (123)";
                ...
                if (accion) {
                    ...
                    seleccionarFila(String.valueOf(grupo.getId()), columnaId);
                }
            }
    ...
    protected void accionBotonAgregarModificar() {// Editando grupo seleccionada de la tabla
        try {
            ...
            boolean valido = puedeInsertarOModificar(true, grupo);
            if (valido) {
                deshacerFiltroTablaGeneral();

                boolean accion = grupo.modificar(usuario);
                cargarDatosGeneral();

                String mensaje = null, titulo = null, tipo = null;
                if (accion) {
                    mensaje = "Se ha modificado satisfactoriamente el grupo.";
                    titulo = "Modificado";
                    tipo = GUIJOption.mensajeInformacion;
                } else {
                    mensaje = "Ha ocurrido un error y no se ha podido modificar el grupo. Por favor inténtelo de nuevo.";
                    titulo = "Error (124)";
                    tipo = GUIJOption.mensajeAdvertencia;
                }
                option.tipoMensaje(tipo, "", titulo, mensaje);
    ...
    private GrupoProducto setValoresGrupo() {
        GrupoProducto grupo = new GrupoProducto();

        grupo.setId(idGrupo);

        grupo.setCodigo(txt_codigo.getText().trim());
        grupo.setNombre(txt_nombre.getText().trim());
    ...
    private boolean puedeInsertarOModificar(boolean esModificar, GrupoProducto grupo) throws ExpendioException {
        String campos = grupo.validarCamposObligatorios(esModificar);

        boolean correcto = true;

        if (campos != null) {
            correcto = correcto && validarCampo(campos, "ID", "Por favor seleccione un grupo.", txt_codigo);
        ...
        correcto = correcto && validarExiste(esModificar, grupo);

        return correcto;
    }

    private boolean validarExiste(boolean esModificar, GrupoProducto grupo) throws ExpendioException {
        boolean valido = !grupo.validarExiste(esModificar);
        if (!valido) {
            option.tipoMensaje(GUIJOption.mensajeAdvertencia, "", "", "Ya existe un código asociado a este grupo.");
    ...
    protected void activarModificar() {// Boton Modificar
        ...
        if (selected != -1) {
            ...
            idGrupo = Long.parseLong((String) dtmTablaGeneral.getValueAt(model, columnaId));
    ...
    protected void accionBotonModificar() {
        if (!tablaGeneral.isRowSelected(tablaGeneral.getSelectedRow())) {
            option.tipoMensaje(GUIJOption.mensajeInformacion, "", "Error", "No ha seleccionado ningún grupo para modificar.");
        } else {
            desactivarBusqueda();
            activarModificar();
        }
    ...
    protected void accionBotonBorrar() {// Boton Borrar
        int selected = tablaGeneral.getSelectedRow();
        if (selected == -1) {
            option.tipoMensaje(GUIJOption.mensajeInformacion, "", "Error", "Por favor seleccione el grupo que desea eliminar.");
        } else {
            ...
            int respuesta = option.tipoMensaje(GUIJOption.mensajePregunta, "", "", "¿Está seguro que desea eliminar el grupo seleccionado?");
            if (respuesta == JOptionPane.YES_OPTION) {
                try {
                    ...
                    boolean accion = grupo.borrar(usuario);
                    cargarDatosGeneral();

                    String mensaje = null, titulo = null, tipo = null;
                    if (accion) {
                        mensaje = "Se ha eliminado satisfactoriamente el grupo.";
                        titulo = "Eliminado";
                        tipo = GUIJOption.mensajeInformacion;
                    } else {
                        mensaje = "Ha ocurrido un error y no se ha podido eliminar el grupo. Por favor inténtelo de nuevo.";
                        titulo = "Error (126)";
                        tipo = GUIJOption.mensajeAdvertencia;
                    }
                    option.tipoMensaje(tipo, "", titulo, mensaje);
